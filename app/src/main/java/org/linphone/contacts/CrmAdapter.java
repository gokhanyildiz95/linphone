...
public class CrmAdapter extends SelectableAdapter<ContactViewHolder> implements SectionIndexer {
    private ArrayList<CrmContacts> mContacts;
    ...
    CrmAdapter(
            Context context,
            ArrayList<CrmContacts> contactsList,
    ...
    public void onBindViewHolder(@NonNull ContactViewHolder holder, final int position) {
        CrmContacts contact = (CrmContacts) getItem(position);
    ...
    public void updateDataSet(ArrayList<CrmContacts> contactsList) {
        mContacts = contactsList;

        mMap = new LinkedHashMap<>();
        String prevLetter = null;
        for (int i = 0; i < mContacts.size(); i++) {
            CrmContacts contact = mContacts.get(i);
            String fullName = contact.getName();
            if (fullName == null || fullName.isEmpty()) {
                continue;
            }
    ...
    public int getSectionForPosition(int position) {
        ...
        CrmContacts contact = mContacts.get(position);
        String fullName = contact.getName();
        if (fullName == null || fullName.isEmpty()) {
            return 0;
        }
