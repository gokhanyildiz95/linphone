...
public class CallManager {
    ...
    public void newOutgoingCall(String to, String displayName) {
        if (to == null) return;
        sysLog.d("MOBILOG", "[umut] to " + to);
        final String pbx_hostname = databaseHelper.getFSSERVERDOMAIN();
        if (to.length() <= 4) {
            to = "<sip:" + to + "@" + pbx_hostname + ">";
        }
        ...
        sysLog.d("MOBILOG", "[umut] after to " + to + "displayname " + displayName);
        ...
        sysLog.d("MOBILOG", "[umut] adress - " + address);
