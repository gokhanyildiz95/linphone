...
public class NotificationBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (intent.getAction().equals(Compatibility.INTENT_REPLY_NOTIF_ACTION)
            ...
            if (core == null) {
                Log.e("[Notification Broadcast Receiver] Couldn't get Core instance");
                sysLog.d("UMUT", "[Notification Broadcast Receiver] Couldn't get Core instance");
                return;
            }
            Call[] calls = core.getCalls();
            for (Call xcall : calls) {
                sysLog.d("UMUT", "call " + xcall.getRemoteAddress().asStringUriOnly());
                String callerAdress = xcall.getRemoteAddress().asStringUriOnly();
                if (callerAdress.contains(remoteAddr)) {
                    sysLog.d("UMUT", "contains call checked");
                    call = xcall;
                }
            }
